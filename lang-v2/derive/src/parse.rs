...
pub fn parse_field(
    ...
    // is also the dup target of another account, so we only fire after
    // the field has loaded as `Some`. Non-`Option`
    ...
    let dup_check = if attrs.is_mut && !attrs.is_dup && is_optional {
        Some(quote! {
            if #field_name.is_some() && __dups.get((__base_offset + #offset_expr) as u8) {
