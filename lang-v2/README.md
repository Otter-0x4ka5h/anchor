...
| `Sysvar<T>` | `Sysvar<Clock>`, `Sysvar<Rent>`, `Sysvar<Instructions>`. Prefer `Clock::get()` / `Rent::get()` syscalls where possible; the instructions sysvar is loaded from account data. (v1 compat) |
